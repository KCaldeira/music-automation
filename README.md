...
- **Tracks 0 … `num_tracks` − 1** each contain the notes generated for one of the requested tracks.
...
- Tracks 0 … N−1 — forward tracks
- Tracks N … 2N−1 — reversed tracks (track *N+i* is the reversed companion of track *i*)
...
| Field | Meaning |
|---|---|
| `beats_per_bar`, `divisions_per_beat`, `bars_per_cycle` | Same as the stochastic workflow (grid + timing). |
